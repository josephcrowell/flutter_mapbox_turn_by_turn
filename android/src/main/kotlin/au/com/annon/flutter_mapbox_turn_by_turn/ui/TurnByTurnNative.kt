...
open class TurnByTurnNative(
    ...
    private val tileStoreObserver =
        object : TileStoreObserver {
            override fun onRegionLoadProgress(
                id: String,
            ...
            override fun onRegionGeometryChanged(
                id: String,
                geometry: Geometry,
            ) {
                mapboxTurnByTurnEvents?.sendJsonEvent(
                    MapboxEventType.TILE_REGION_GEOMETRY_CHANGED,
                    "{\"id\":\"${id}\"}",
                )
            }

            override fun onRegionMetadataChanged(
                id: String,
                value: Value,
            ) {
                mapboxTurnByTurnEvents?.sendJsonEvent(
                    MapboxEventType.TILE_REGION_METADATA_CHANGED,
                    "{\"id\":\"${id}\"}",
                )
            }
        }
    ...
    fun initializeMapbox() {
        ...
        viewportDataSource!!
            .options.followingFrameOptions.bearingSmoothing.enabled = false
    ...
    private fun unregisterObservers() {
        // Use try-catch because MapboxNavigationApp may have already been disabled
        try {
            if (navigationStarted) {
                clearRouteAndStopNavigation()
            }
            ...
            Log.w("TurnByTurnNative", "MapboxNavigation already disabled: ${e.message}")
        }
        ...
        navigationBasicGesturesHandler?.let {
            binding.mapView.camera.removeCameraAnimationsLifecycleListener(it)
        }
    ...
    override fun onDestroy() {
        // Remove tileStore observer FIRST - this is held by native JNI references
        // and must be removed before any other cleanup
        tileStore?.removeObserver(tileStoreObserver)
        ...
        eventChannel = null
        messenger = null
        mapboxTurnByTurnEvents = null

        // Cancel all Mapbox APIs to release native resources
        maneuverApi.cancel()
        routeLineApi.cancel()
    ...
    private fun startNavigation(call: MethodCall) {
        ...
        if (waypointList.isNotEmpty() && waypointNamesList.isNotEmpty()) {
            run {
    ...
    private fun addOfflineMap(call: MethodCall) {
        ...
        val stylePackLoadOptions =
            StylePackLoadOptions
                .Builder()
                .glyphsRasterizationMode(GlyphsRasterizationMode.IDEOGRAPHS_RASTERIZED_LOCALLY)
                .build()

        val mapsTilesetDescriptor =
